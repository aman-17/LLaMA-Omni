...
class AdaptiveMultiturnSpeechDataset(Dataset):
    def __init__(self, questions, tokenizer, model_config):
        self.questions = questions
        self.tokenizer = tokenizer
        self.model_config = model_config
        
        # Detect whisper model type from config
        self.whisper_model_type = getattr(model_config, 'speech_encoder', 'large-v3')
        self.mel_size = self._get_mel_size_for_model(self.whisper_model_type)
        
    def _get_mel_size_for_model(self, model_name):
        """Determine mel size based on whisper model type"""
        if 'tiny' in model_name.lower():
            ...
            # Default to 128 for unknown models (safer for larger models)
            return 128
    ...
    def process_messages(self, messages):
        ...
        input_ids[
            input_ids == self.tokenizer.convert_tokens_to_ids(DEFAULT_SPEECH_TOKEN)
        ] = SPEECH_TOKEN_INDEX
        return input_ids
    ...
    def __getitem__(self, index):
        ...
        return {"input_ids": input_ids, "speech": speech_list}
...
def collate_fn(batch):
    ...
    speech_tensors = torch.nn.utils.rnn.pad_sequence(
        all_speech, batch_first=True, padding_value=0
    )
...
def create_data_loader(questions, tokenizer, model_config, batch_size=1, num_workers=0):
    ...
    dataset = AdaptiveMultiturnSpeechDataset(questions, tokenizer, model_config)
...
def eval_model(args):
    ...
    for (input_ids, speech_tensor, speech_lengths), item in tqdm(
        ...
        if args.s2s:
            ...
            ans_file.write(
                json.dumps(
                    {"question_id": idx, "prediction": output_text}, ensure_ascii=False
                )
                + "\n"
            )
