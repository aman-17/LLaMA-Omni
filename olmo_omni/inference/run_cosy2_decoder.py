...
class SpeechDecoder:
    def __init__(
        ...
        self.flow.load_state_dict(
            torch.load("{}/flow.pt".format(model_dir), map_location=self.device),
            strict=True,
        )
        ...
        hift_state_dict = {
            ...
            for k, v in torch.load(
                "{}/hift.pt".format(model_dir), map_location=self.device
            ).items()
        }
        ...
        self.token_hop_len = (
            hop_len if hop_len is not None else 2 * self.flow.input_frame_rate
        )
    ...
    def entry(self, generated_tokens, prompt_speech_16k, stream=False, speed=1.0):
        ...
        if stream:
            ...
            while True:
                if (
                    len(generated_tokens) - token_offset
