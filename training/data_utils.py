...
from olmo_omni.constants import DEFAULT_SPEECH_TOKEN, IGNORE_INDEX
from olmo_omni.datasets.preprocess import preprocess, preprocess_multimodal
...
class InstructS2SDataset(Dataset):
    def __init__(
        ...
        if stage == 2:
            assert (
                kmeans_model_path is not None
            ), "K-means model path required for stage 2"
                ...
                for item in raw_data:
                    ...
                    for i in range(0, len(conversation), 2):
                        if i + 1 < len(conversation):
                            ...
                            if (
                                human_entry["from"] == "human"
                                and gpt_entry["from"] == "gpt"
                            ):
                                speech_instruction_path = human_entry["speech"].replace(
                                    "data/multiturn_instruction/en/", ""
                                )
                                speech_response_path = gpt_entry["speech"].replace(
                                    "data/multiturn_instruction/en/", ""
                                )
                                if os.path.exists(
                                    os.path.join(
                                        os.path.dirname(self.data_path),
                                        speech_instruction_path,
                                    )
                                ) and os.path.exists(
                                    os.path.join(
                                        os.path.dirname(self.data_path),
                                        speech_response_path,
                                    )
                                ):
                                    data.append(
                                        {
    ...
    def load_audio(self, audio_path: str) -> torch.Tensor:
        if self.data_args.input_type == "mel":
            ...
            n_mels = getattr(self.data_args, "mel_size", 80)
            mel = whisper.log_mel_spectrogram(audio, n_mels=n_mels)
            # whisper.log_mel_spectrogram returns [n_mels, time]
            mel = mel.permute(1, 0)  # [time, n_mels]
            return mel
        else:
            audio = whisper.load_audio(audio_path)
            audio = whisper.pad_or_trim(audio)
    ...
    def extract_speech_units(
        self, units_string: str = None, audio_path: str = None
    ) -> List[int]:
        if units_string:
            ...
            units = [units[0]] + [
                units[i] for i in range(1, len(units)) if units[i] != units[i - 1]
            ]
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        speech_path = os.path.join(
            os.path.dirname(self.data_path), item["speech_instruction_path"]
        )
        ...
        if self.stage == 2:
            ...
            if "speech_units" in item and item["speech_units"]:
                speech_units = self.extract_speech_units(
                    units_string=item["speech_units"]
                )
            elif "speech_response_path" in item and item["speech_response_path"]:
                response_audio_path = os.path.join(
                    os.path.dirname(self.data_path), item["speech_response_path"]
                )
            ...
            if "speech_units" in instances[0]:
                ...
                for units in speech_units:
                    ...
                    if units.size(0) < max_unit_len:
                        padding = torch.full(
                            (max_unit_len - units.size(0),), -1, dtype=torch.long
                        )
