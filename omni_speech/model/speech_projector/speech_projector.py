...
class ConvAttentionProjector(nn.Module):
    def __init__(self, config):
        ...
        self.conv1d = nn.Conv1d(self.encoder_dim, self.encoder_dim,
                                kernel_size=self.compression_ratio,
                                stride=self.compression_ratio, padding=0)
        self.attention = nn.MultiheadAttention(self.encoder_dim, num_heads=8, batch_first=True)
...
class HierarchicalPoolingProjector(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_weights = self.attention_pool(x_chunks.view(batch_size * num_chunks, self.k, dim))
        attention_pooled = (x_chunks.view(batch_size * num_chunks, self.k, dim) *
...
class LearnableTokenCompressor(nn.Module):
    def __init__(self, config):
        ...
        self.compress_tokens = nn.Parameter(torch.randn(1, self.num_compress_tokens, self.encoder_dim))
        self.cross_attention = nn.MultiheadAttention(self.encoder_dim, num_heads=8, batch_first=True)
...
class HiggsStyleAudioProjector(nn.Module):    
    def __init__(self, config):
        ...
        if self.use_compression:
            self.compression_type = getattr(config, 'compression_type', 'attention_pool')
            ...
            if self.compression_type == 'attention_pool':
                ...
                self.conv_compress = nn.Conv1d(
                    ...
                    kernel_size=self.k, stride=self.k, padding=0
                )
            elif self.compression_type == 'adaptive_pool':
                self.adaptive_pool = nn.AdaptiveAvgPool1d(config.max_speech_tokens // self.k)
...
class MultiScaleAudioProjector(nn.Module):
    def __init__(self, config):
        ...
        self.scale_projectors = nn.ModuleList([
            nn.Linear(self.encoder_dim * scale, self.llm_dim) 
    ...
    def forward(self, x):
        ...
        for i, scale in enumerate(self.scales):
            if seq_len % scale == 0:
                ...
                x_scale = x_padded.view(batch_size, (seq_len + pad_len) // scale, dim * scale)
                projected = self.scale_projectors[i](x_scale)
                scale_outputs.append(projected)
